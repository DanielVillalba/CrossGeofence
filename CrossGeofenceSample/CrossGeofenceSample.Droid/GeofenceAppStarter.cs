...
namespace CrossGeofenceSample.Droid
{
    ...
    public class GeofenceAppStarter : Application
    {
        ...
        public override void OnCreate()
        {
            ...
            CrossGeofence.Initialize<CrossGeofenceSample.Helpers.CrossGeofenceListener>();
        ...
        {
            AppContext.StartService(new Intent(AppContext, typeof(Helpers.GeofenceService)));

            if (Android.OS.Build.VERSION.SdkInt >= Android.OS.BuildVersionCodes.Kitkat)
            {

                PendingIntent pintent = PendingIntent.GetService(AppContext, 0, new Intent(AppContext, typeof(Helpers.GeofenceService)), 0);
        ...
        {
            AppContext.StopService(new Intent(AppContext, typeof(Helpers.GeofenceService)));
            if (Android.OS.Build.VERSION.SdkInt >= Android.OS.BuildVersionCodes.Kitkat)
            {
                PendingIntent pintent = PendingIntent.GetService(AppContext, 0, new Intent(AppContext, typeof(Helpers.GeofenceService)), 0);
                AlarmManager alarm = (AlarmManager)AppContext.GetSystemService(Context.AlarmService);
